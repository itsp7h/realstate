...
table.ledger tr.closing-row td { border-top: 1.5px solid #111827; border-bottom: none; font-weight: 700; padding-top: 8px; }
.owing { color: #DC2626; font-weight: 700; }
.settled { color: #059669; font-weight: 700; }
...
<div class="report-title">TENANT LEDGER</div>
...
<table class="ledger">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="right">{{ $row['debit'] > 0.001 ? number_format($row['debit'], 3) : '—' }}</td>
            <td class="right">{{ $row['credit'] > 0.001 ? number_format($row['credit'], 3) : '—' }}</td>
            <td class="right {{ $row['balance'] > 0.001 ? 'owing' : 'settled' }}">
                {{ number_format(abs($row['balance']), 3) }}{{ $row['balance'] > 0.001 ? ' Dr' : ($row['balance'] < -0.001 ? ' Cr' : '') }}
            </td>
        </tr>
        @empty
        <tr><td colspan="6" style="text-align:center;padding:20px;color:#64748B">No transactions in this date range.</td></tr>
        @endforelse
        @if($rows->isNotEmpty())
...
<div class="footer-note">This is a computer-generated ledger. "On Account" credits and post-dated cheques are not reflected.</div>
